...
void DrawMenu(double winwidth, double winheight)
{
	...
	double MenuButtonInterval = 0.6;// 按钮之间的间隔
	...
	int    FileSelection, EditSelection, ModeSelection, TextSelection, GraphicSelection,
		LineSelection, MusicSelection, HelpSelection;// 分别记录每一个按钮的返回值
	static int IsInText = 0;// 标志是否处于文本模式，以变动文本菜单按钮中的内容
	...
	menuListText[1] = IsInText ? "退出文本" : "插入文本";// 根据是否处于文本模式变化内容
	...
	if (TextSelection > 40 && CreateMode == 1) {// 当且仅当处于文本模式，这个菜单栏下的按钮才会有响应
		...
		case 41:
			IsInText = IsInText ? 0 : 1;// 点击第一个按钮，转变IsInText，继而转变所处模式
...
void DrawIconButton(double winwidth, double winheight) {
	...
	static int	  ButtonSelection[5] = { 0,0,0,0,0 };// 记录选择
